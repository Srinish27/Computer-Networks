import socket

recieverIP = "10.0.0.2"
recieverPort   = 20002
bufferSize  = 1024 #Message Buffer Size


# Create a UDP socket
socket_udp = socket.socket(family=socket.AF_INET, type=socket.SOCK_DGRAM)

# Bind socket to localIP and localPort
socket_udp.bind((recieverIP, recieverPort))

print("UDP socket created successfully....." )
recieved_image = open("received.jpg","wb")
sequence = 0
while True:
    
    #wait to recieve message from the server
    bytesAddressPair = socket_udp.recvfrom(bufferSize)
    #split the recieved tuple into variables
    recievedMessage = bytesAddressPair[0]
    senderAddress = bytesAddressPair[1]
    recv_byte = int.from_bytes(recievedMessage[:2],'big')
    #print(sequence,recv_byte)
    if(recv_byte==sequence): 
        recieved_image.write(recievedMessage[2:])
        socket_udp.sendto(recievedMessage[:2],senderAddress)
        sequence += 1
    elif(recv_byte<sequence):
       socket_udp.sendto((sequence-1).to_bytes(2,'big'),senderAddress)
    
    #print them just for understanding